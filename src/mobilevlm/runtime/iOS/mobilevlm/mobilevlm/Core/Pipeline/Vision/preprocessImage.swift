...
func preprocessImage(
    ...
    let meanR: Float = 0.48145466
    let meanG: Float = 0.4578275
    let meanB: Float = 0.40821073

    let stdR: Float = 0.26862954
    let stdG: Float = 0.26130258
    let stdB: Float = 0.27577711

    // background color
    let bgR = Float(UInt8(floor(meanR * 255.0)))
    let bgG = Float(UInt8(floor(meanG * 255.0)))
    let bgB = Float(UInt8(floor(meanB * 255.0)))
    ...
    // resize scale
    let scaleX = Float(squareSize) / Float(targetSize)
    let scaleY = Float(squareSize) / Float(targetSize)
    ...
    for y in 0..<targetSize {
        ...
        for x in 0..<targetSize {
            ...
            func sample(_ sx: Int, _ sy: Int) -> (Float, Float, Float) {
                ...
                return (
                    Float(rgba[rgbaIndex]),
                    Float(rgba[rgbaIndex + 1]),
                    Float(rgba[rgbaIndex + 2])
                )
            }

...
# retrieve our model from the ONNX model zoo and download local file
urllib.request.urlretrieve(onnx_model_url, filename="resnet50v2.tar.gz")
urllib.request.urlretrieve(imagenet_labels_url, filename="imagenet-simple-labels.json")

# (bash) tar xvzf resnet50v2.tar.gz
...
outputs = [session.run(
    None,  # Return all output nodes
    {input_name: inputs[i]}  # {graph input node name: numpy input tensor}
)[0] for i in range(test_data_num)]
